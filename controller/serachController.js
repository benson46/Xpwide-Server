  ...
  try {
    ...
    switch (entity) {
      ...
      case 'products':
        ...
        break;
      case 'orders':
        // Search orders by order number (adjust field as needed)
        results = await Order.find({ orderNumber: { $regex: regex } });
        break;
      case 'categories':
        // Search categories by name
        results = await Category.find({ name: { $regex: regex } });
        break;
      case 'brands':
        // Search brands by name
        results = await Brand.find({ name: { $regex: regex } });
        break;
      case 'coupons':
        ...
        case 'offers':
            ...
            results = await Offer.find(filter);
            break;
